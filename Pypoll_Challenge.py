...
with open(file_to_load) as election_data:
    ...
    for row in reader:
        ...
        if county_name not in counties_and_votes:
            ...
            counties_and_votes[county_name] = 0

        # 5: Add a vote to that county's vote count.
        counties_and_votes[county_name] += 1
...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in counties_and_votes:
        # 6b: Retrieve the county vote count.
        county_votes = counties_and_votes[county_name]
        # 6c: Calculate the percentage of votes for the county.
        county_vote_percentage = float(county_votes) / float(total_votes) * 100

         # 6d: Print the county results to the terminal.
        print(f"{county_name}: received {county_vote_percentage:.1f}% of the vote.\n")
         # 6e: Save the county votes to a text file.
    #with open(file_to_save, "w") as txt_file:
        county_voting_results = (
            f"\nCounty Votes:"
            f"{county_name}: {county_vote_percentage:.2f}% ({county_votes:,})"
            )
